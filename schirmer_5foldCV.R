...
USE_SYNTHETIC_CHECK <- FALSE

if (USE_SYNTHETIC_CHECK) {
  ...
  cat("Dropped", sum(!complete.cases(synthetic_check_data)), "incomplete rows from synthetic data.\n")
  
  X     <- synthetic_check_data[, grep("^taxon_", colnames(synthetic_check_data))]
  y_age <- synthetic_check_data$age
  
  # NOTE: the synthetic data only has a planted AGE signal.
  # BMI and Sex below are just placeholders so the rest of the
  # script doesn't crash -- their results are meaningless here.
  # This check is ONLY valid for the "Age" rows in your output.
  y_bmi <- synthetic_check_data$age
  y_sex <- sample(c("male", "female"), nrow(X), replace = TRUE)
  
  cat("*** RUNNING ON SYNTHETIC SANITY-CHECK DATA -- Age task only is meaningful ***\n")
}
## ---- end temporary block ----
